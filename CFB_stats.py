...
class CFB:
    ...
    def rank(self):
        """Obtains ranking of conferences based on Sports Reference ranking."""
    ...
    def data_input(self):
        ...
        # Loop through all years in range. Obtain schedule and ranking for given year.
        for year in range(self.year1,self.year2 + 1):
            schedule = CFB(year,self.year2).schedule()
            rank = CFB(year,self.year2).rank()
            
            # Inititialize conference stats and loop through conferences, obtaining stats and ranking for each.
            # Conference ranking used to determine strength of schedule for each team.
            all_conf_stats = pd.DataFrame()
            for conf in rank['Conference']:
                stats = CFB(year,self.year2).CFB_stats(conf)
                stats['Conf Rank'] = [rank[rank['Conference'] == conf].iloc[0,0] for x in range(len(stats))]
                
                # Concatenate stats from each conference.
                all_conf_stats = pd.concat([all_conf_stats,stats])
                
            # Compile awar and home stats, and merge the two. Indicate when given year's data is compiled.
    ...
    def pred_input(self,pred_year,games):
        ...
        # Initialize dataframe for storing current year stats on teams playing. Also obtain conference 
        # rankings and initialize number of game to update user on progress.
        all_games = pd.DataFrame()
        rank = CFB(pred_year,self.year2).rank()
        game_num = 1
        ...
        for game in games:
            ...
            # Format stats using home and away.
            away_stats = away_conf_stats[away_conf_stats['School'] == game[0]]                .copy().reset_index().drop(columns='index')
            away_stats['Conf Rank'] = rank[rank['Conference'] == away_conf].iloc[0,0] 
            for col in away_stats.columns:
                away_stats.rename(columns={col: 'Away {}'.format(col)},inplace=True)   
            home_stats = home_conf_stats[home_conf_stats['School'] == game[1]]                .copy().reset_index().drop(columns='index')
            home_stats['Conf Rank'] = rank[rank['Conference'] == home_conf].iloc[0,0] 
